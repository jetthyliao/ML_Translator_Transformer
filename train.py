...
#from Models import get_modeli

#from Batch import create_maskss
...
def create_dataset(options, SOURCE, TARGET):
    ...
    torch.device = options["device"]

    train_iter = MyIterator(train, batch_size=options['batchsize'], repeat=False, sort_key=lambda x: (len(x.source), len(x.target)), batch_size_fn=batch_size_fn, train=True, shuffle=True)
...
also lads any weights that were created in prior training runs
"""
